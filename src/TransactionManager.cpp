...
static bool hasDuplicateCategoryNames(const std::vector<std::unique_ptr<Category>>& categories) {
    std::unordered_set<std::string> seen;
    for (const auto& c : categories) {
        if (!seen.insert(util::str_to_lower(c->getName())).second) {
            return true;
        }
    }
    return false;
...
std::expected<int, DataManager::DataReadWriteError> TransactionManager::load(const DataManager& dataManager) {
    ...
    if (
        hasDuplicateTransactionIds(transactions)
        || hasDuplicateCategoryIds(categories)
        || hasDuplicateCategoryNames(categories)
    ) {
        this->transactions = std::move(previousTransactions);
        this->categories = std::move(previousCategories);
